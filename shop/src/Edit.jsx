...
export default function EditTask() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await fetch(`http://localhost:8000/api/to-do-list/${id}`);
            const data = await res.json();
            setTitle(data[0].task);
            setDescription(data[0].details);
        }
        fetchData();
    }, [id])
    ...
    async function handleSubmit(event) {
        try {
            ...
            const response = await fetch(`http://localhost:8000/api/to-do-list/${id}`, {
                method: "PATCH",
